...
public class SetIntRecord implements LogRecord {
   ...
   public static int writeToLog(LogMgr lm, int txnum, BlockId blk, int offset, int val) {
      ...
      p.setInt(0, SETINT);                 // AM: Transaction type. The first integer (offset 0) is always the Operator ID (e.g., SETINT = 4). This tells the Recovery Manager how to parse the rest of the bytes.
      ...
      p.setInt(opos, offset);                      // AM: Records offset integer value. Example: If you updated a student's name at byte 40 of Block 100, 40 is recorded as the integer value.
      p.setInt(vpos, val);                         // AM: Writes the old integer record. (Log is setup as an "undo" record type)
